...
def delete_selected_entry():
    ...
    try:
        ...
        if res:
            ...
            delete_query = '''
            DELETE  FROM takebook 
            WHERE studentid=%s AND studentname=%s AND bookid=%s AND bookname=%s AND author=%s
            '''
            mycursor.execute(delete_query, (studentid, studentname, bookid, bookname, author))
            mysqlcon.commit()
...
try:
    ...
    query = "SELECT studentid, studentname, bookid, bookname, author FROM takebook"
...
delete_button = Button(f, text="Delete Selected Entry",border=0, command=delete_selected_entry, bg='#ff7675', fg='black')
